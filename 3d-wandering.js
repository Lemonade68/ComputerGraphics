...
window.onkeydown = function(e){
    switch (e.keyCode) { 
        ...
        case 86: //V-关闭深度缓存，不用消隐
            // 关闭深度缓存，以正确渲染物体被遮挡部分
            gl.disable(gl.DEPTH_TEST); //default
            break;
...
window.onresize = setCanvasAndViewport;

function setCanvasAndViewport(){
...
*三角函数需要用到Math库，MV.js提供的函数radians将三角函数中的角度值转换为弧度值。
************************************************************************/
/***提示2：UP向量的计算**********************************
*通常UP=(0,1,0),但是当相机视线方向n和UP共线时会有问题! 解决方法：找垂直于观察方向n的某线段作为up向量
*目前还没有其它办法，参见"HW3评讲". 或者界定phi的范围：-89~89,保证不会出现up平行于n的情况发生。
*********************************************************/
    ...
    //方法1：在直角三角形，找垂直于观察方向n的向上向量	
	// var Pup = vec3( 2 * radius * Math.sin(radians(phi-60)) * Math.sin(radians(theta)), 
    //           2 * radius * Math.cos(radians(phi-60)),
    //           2 * radius * Math.sin(radians(phi-60)) * Math.cos(radians(theta)));	
    // var up=subtract(Pup,eye); 
    
    //方法2： 让Pup初始位置(0,1,0)，相当于绕X轴逆时针转90度，即phi-90
    //等效于向量up同eye一同进行运动旋转。
    var  up= vec3( radius * Math.sin(radians(phi-90)) * Math.sin(radians(theta)), 
    radius * Math.cos(radians(phi-90)), 
    radius * Math.sin(radians(phi-90)) * Math.cos(radians(theta)));
...
function formProjectMatrix(){
   //==========TODO: 计算投影矩阵=======================
	//提示1：可调用common目录下的MV.js里ortho(),perspective()函数
    ...
    const right = 1.0;
    const bottom = -1.0;
    const ytop = 1.0; // 注意：top是js全局变量，所以这里改名为ytop
